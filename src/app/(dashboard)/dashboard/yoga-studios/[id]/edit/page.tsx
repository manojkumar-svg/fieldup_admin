...
import { yogaStudioSchema, type YogaStudioInput } from '@/lib/validations/entities';
...
export default function EditYogaStudioPage(): React.ReactElement {
  ...
  const { data, isLoading, error, refetch } = useQuery<{ yogaStudio: YogaStudio }>({
    queryKey: ['yoga-studio', studioId],
    queryFn: async () => {
      const res = await fetch(`/api/yoga-studios/${studioId}`);
      if (!res.ok) throw new Error('Failed to fetch yoga studio');
      return res.json();
    },
  });
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<YogaStudioInput>({
    resolver: zodResolver(yogaStudioSchema),
    values: data?.yogaStudio
      ? {
          name: data.yogaStudio.name,
          description: data.yogaStudio.description ?? '',
          address: data.yogaStudio.address,
          city: data.yogaStudio.city,
          state: data.yogaStudio.state,
          pincode: data.yogaStudio.pincode,
          latitude: data.yogaStudio.latitude,
          longitude: data.yogaStudio.longitude,
          contactPhone: data.yogaStudio.contactPhone,
          contactEmail: data.yogaStudio.contactEmail ?? '',
          website: data.yogaStudio.website ?? '',
          amenities: data.yogaStudio.amenities ?? [],
          images: data.yogaStudio.images ?? [],
          documents: data.yogaStudio.documents ?? [],
          imageTitles: data.yogaStudio.imageTitles ?? [],
          documentTitles: data.yogaStudio.documentTitles ?? [],
          openTime: data.yogaStudio.openTime ?? '',
          closeTime: data.yogaStudio.closeTime ?? '',
          monthlyFee: data.yogaStudio.monthlyFee,
        }
      : undefined,
  });

  const updateMutation = useMutation({
    mutationFn: async (formData: YogaStudioInput) => {
      const res = await fetch(`/api/yoga-studios/${studioId}`, {
        method: 'PUT',
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error?.message ?? 'Failed to update yoga studio');
      }
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['yoga-studios'] });
      queryClient.invalidateQueries({ queryKey: ['yoga-studio', studioId] });
      toast('Yoga studio updated successfully', 'success');
      router.push('/dashboard/yoga-studios');
    },
    onError: (err: Error) => {
  ...
  return (
    <div>
      <PageHeader title="Edit Yoga Studio" backHref="/dashboard/yoga-studios" />
                ...
                <Controller
                  ...
                  render={({ field: titlesField }) => (
                    <FileUpload
                      ...
                      accept="image/*"
                      maxFiles={10}
                      hint="Upload photos of the yoga studio (max 10)"
                    />
                  )}
                />
              )}
