...
import { Plus, Search, Flower2, Filter, ToggleLeft, Trash2, CheckSquare } from 'lucide-react';
...
interface YogaStudiosResponse {
  yogaStudios: YogaStudio[];
  total: number;
  page: number;
  totalPages: number;
}

export default function YogaStudiosPage(): React.ReactElement {
  ...
  const { data, isLoading, error, refetch } = useQuery<YogaStudiosResponse>({
    queryKey: ['yoga-studios', search, status, page],
    queryFn: async () => {
      const res = await fetch(`/api/yoga-studios?${queryParams.toString()}`);
      if (!res.ok) throw new Error('Failed to fetch yoga studios');
      return res.json();
    },
  });
  ...
  const toggleStatus = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['yoga-studios'] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`/api/yoga-studios/${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Failed to delete yoga studio');
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['yoga-studios'] });
      toast('Yoga studio deleted', 'success');
      setDeleteId(null);
    },
    onError: () => {
      toast('Failed to delete yoga studio', 'error');
    },
  });
  ...
  const bulkStatusMutation = useMutation({
    mutationFn: async (newStatus: string) => {
      const res = await fetch('/api/bulk/status', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ entityType: 'yoga_studios', ids: Array.from(selectedIds), status: newStatus }),
      });
    ...
    onSuccess: (_, newStatus) => {
      queryClient.invalidateQueries({ queryKey: ['yoga-studios'] });
      toast(`${selectedIds.size} yoga studios ${newStatus === 'ACTIVE' ? 'activated' : 'deactivated'}`, 'success');
      setSelectedIds(new Set());
    },
    onError: () => {
      toast('Failed to update yoga studios', 'error');
    },
  });
  ...
  const toggleSelectAll = () => {
    if (!data) return;
    if (selectedIds.size === data.yogaStudios.length) {
      setSelectedIds(new Set());
    } else {
      setSelectedIds(new Set(data.yogaStudios.map((y) => y.id)));
    }
  };
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Yoga Studios"
        subtitle={data ? `${data.total} total yoga studios` : undefined}
        action={
          <Link href="/dashboard/yoga-studios/new">
            <Button>
      ...
      {/* Empty */}
      {!isLoading && (!data || data.yogaStudios.length === 0) && (
        <EmptyState
          icon={<Flower2 className="h-10 w-10" />}
          title="No yoga studios found"
          message="Get started by adding your first yoga studio."
          actionLabel="Add Yoga Studio"
          onAction={() => router.push('/dashboard/yoga-studios/new')}
        />
      )}

      {data && data.yogaStudios.length > 0 && (
        <>
          {/* Bulk Action Bar */}
          {selectedIds.size > 0 && (
              ...
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  <th className="px-3 py-3.5 text-left">
                    <input
                      type="checkbox"
                      checked={data.yogaStudios.length > 0 && selectedIds.size === data.yogaStudios.length}
              ...
              <tbody>
                {data.yogaStudios.map((studio) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-sm text-gray-500">{studio.city}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-500">{studio.contactPhone}</td>
                    <td className="px-5 py-3.5 text-sm font-medium text-gray-700">
                      {studio.monthlyFee != null ? `${formatCurrency(studio.monthlyFee)}/mo` : '—'}
                    </td>
                    <td className="px-5 py-3.5">
                      ...
                      <span className="flex items-center justify-end gap-1">
                        <Button
                          ...
                          onClick={() =>
                            toggleStatus.mutate({
                              id: studio.id,
                              newStatus: studio.status === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE',
                            })
                          }
                        >
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h3 className="text-sm font-semibold text-gray-900 truncate">{studio.name}</h3>
                      <p className="text-xs text-gray-500 mt-0.5">{studio.city}</p>
                    </div>
                  </div>
                  <StatusBadge status={studio.status} />
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 text-xs text-gray-500">
                    ...
                    {studio.monthlyFee != null && (
                      <span className="font-medium text-gray-700">{formatCurrency(studio.monthlyFee)}/mo</span>
                    )}
                  </div>
                  <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                    <button
                      ...
                      onClick={() => toggleStatus.mutate({ id: studio.id, newStatus: studio.status === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE' })}
      ...
      <ConfirmDialog
        ...
        title="Delete Yoga Studio"
        message="Are you sure you want to delete this yoga studio? This action cannot be undone."
