...
export default function NewZumbaStudioPage(): React.ReactElement {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: ZumbaStudioInput) => {
      const res = await fetch('/api/zumba-studios', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error?.message ?? 'Failed to create zumba studio');
      }
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['zumba-studios'] });
      toast('Zumba studio created successfully', 'success');
  ...
  return (
    <div>
      <PageHeader title="Add Zumba Studio" backHref="/dashboard/zumba-studios" />
