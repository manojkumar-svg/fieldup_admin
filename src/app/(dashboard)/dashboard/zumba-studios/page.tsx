...
import { Plus, Search, Music, Filter, ToggleLeft, Trash2, CheckSquare } from 'lucide-react';
...
interface ZumbaStudiosResponse {
...
export default function ZumbaStudiosPage(): React.ReactElement {
  ...
  const { data, isLoading, error, refetch } = useQuery<ZumbaStudiosResponse>({
    queryKey: ['zumba-studios', search, status, page],
    queryFn: async () => {
      const res = await fetch(`/api/zumba-studios?${queryParams.toString()}`);
      if (!res.ok) throw new Error('Failed to fetch zumba studios');
      return res.json();
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`/api/zumba-studios/${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Failed to delete zumba studio');
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['zumba-studios'] });
      toast('Zumba studio deleted', 'success');
      setDeleteId(null);
    },
    onError: () => {
      toast('Failed to delete zumba studio', 'error');
    },
  });
  ...
  const bulkStatusMutation = useMutation({
    mutationFn: async (newStatus: string) => {
      const res = await fetch('/api/bulk/status', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ entityType: 'zumba_studios', ids: Array.from(selectedIds), status: newStatus }),
      });
    ...
    onSuccess: (_, newStatus) => {
      queryClient.invalidateQueries({ queryKey: ['zumba-studios'] });
      toast(`${selectedIds.size} zumba studios ${newStatus === 'ACTIVE' ? 'activated' : 'deactivated'}`, 'success');
      setSelectedIds(new Set());
    },
    onError: () => {
      toast('Failed to update zumba studios', 'error');
    },
  });
  ...
  const toggleSelectAll = () => {
    if (!data) return;
    if (selectedIds.size === data.zumbaStudios.length) {
      setSelectedIds(new Set());
    } else {
      setSelectedIds(new Set(data.zumbaStudios.map((z) => z.id)));
    }
  };
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Zumba Studios"
        subtitle={data ? `${data.total} total zumba studios` : undefined}
        action={
          <Link href="/dashboard/zumba-studios/new">
            <Button>
      ...
      <div className="space-y-3 mb-6">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search zumba studios..."
              value={search}
      ...
      {/* Empty */}
      {!isLoading && (!data || data.zumbaStudios.length === 0) && (
        <EmptyState
          icon={<Music className="h-10 w-10" />}
          title="No zumba studios found"
          message="Get started by adding your first zumba studio."
          actionLabel="Add Zumba Studio"
          onAction={() => router.push('/dashboard/zumba-studios/new')}
        />
      )}

      {data && data.zumbaStudios.length > 0 && (
        <>
          {/* Bulk Action Bar */}
          {selectedIds.size > 0 && (
              ...
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  <th className="px-3 py-3.5 text-left">
                    <input
                      type="checkbox"
                      checked={data.zumbaStudios.length > 0 && selectedIds.size === data.zumbaStudios.length}
      ...
      <ConfirmDialog
        ...
        title="Delete Zumba Studio"
        message="Are you sure you want to delete this zumba studio? This action cannot be undone."
