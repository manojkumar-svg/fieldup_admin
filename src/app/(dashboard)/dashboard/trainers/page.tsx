...
export default function TrainersPage(): React.ReactElement {
  ...
  const { data, isLoading, error, refetch } = useQuery<TrainersResponse>({
    queryKey: ['trainers', search, sportType, status, page],
    queryFn: async () => {
      const res = await fetch(`/api/trainers?${queryParams.toString()}`);
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Trainers"
        subtitle={data ? `${data.total} total trainers` : undefined}
        action={
      ...
      {/* Empty */}
      {!isLoading && (!data || data.trainers.length === 0) && (
        <EmptyState
          icon={<Users className="h-10 w-10" />}
          title="No trainers found"
          message="Get started by adding your first trainer."
          actionLabel="Add Trainer"
          onAction={() => router.push('/dashboard/trainers/new')}
        />
      )}
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  ...
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Hourly Rate</th>
              ...
              <tbody>
                {data.trainers.map((trainer) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-sm text-gray-500">{trainer.experience} yrs</td>
                    <td className="px-5 py-3.5 text-sm font-medium text-gray-700">₹{trainer.hourlyRate}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-500">{trainer.city}</td>
                    <td className="px-5 py-3.5">
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 text-xs text-gray-500">
                    ...
                    <span className="font-medium text-gray-700">₹{trainer.hourlyRate}/hr</span>
                  </div>
                  <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                    <button
                      ...
                      onClick={() => toggleStatus.mutate({ id: trainer.id, newStatus: trainer.status === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE' })}
